...
<ParamField path="visitorId" type="string">
  Default browser continuity identifier. Obtain it from the Era Web SDK.
</ParamField>

<ParamField path="partnerEndUserId" type="string">
  Default stable partner identifier for an authenticated user. Supply it only from a trusted backend.
</ParamField>

<ParamField path="email" type="string">
  Default trusted email bridge assertion. Requires `partnerEndUserId`.
</ParamField>

<ParamField path="phone" type="string">
  Default trusted phone bridge assertion. Requires `partnerEndUserId`.
</ParamField>
...
<ParamField path="mode" type='"async" | "sync"' default='"async"'>
  Operating mode. `async` returns the previous turn's precomputed block after an API round trip. `sync` waits for the current turn's computation. See [Async vs Sync Modes](/concepts/async-sync).
</ParamField>
...
At least `visitorId` or `partnerEndUserId` must resolve from the call or constructor defaults. Email and phone bridge assertions require `partnerEndUserId` and must come from a trusted backend.
